...
// Импорт flutter_dotenv для работы с .env файлами
...
class ChatProvider with ChangeNotifier {
  ...
  // Количество бесплатных моделей
  int _freeModelsCount = 0;
  // Флаг загрузки
  bool _isLoading = false;
  // Версия данных для обновления UI
  int _dataVersion = 0;
  ...
  // Геттер для количества бесплатных моделей
  int get freeModelsCount => _freeModelsCount;
  // Геттер для версии данных
  ...
  // Метод загрузки настроек из SharedPreferences
  Future<void> loadSettings() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final savedApiKey =
          prefs.getString('api_key') ?? dotenv.env['OPENROUTER_API_KEY'] ?? '';
      final savedBaseUrl =
          prefs.getString('base_url') ?? dotenv.env['BASE_URL'] ?? '';
      _api.updateSettings(savedApiKey, savedBaseUrl);
      _log('Settings loaded: baseUrl = $savedBaseUrl');
    } catch (e) {
      _log('Error loading settings: $e');
    }
  }

  // Метод сохранения настроек в SharedPreferences
  Future<void> saveSettings(String apiKey, String baseUrl) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString('api_key', apiKey);
      await prefs.setString('base_url', baseUrl);
      _api.updateSettings(apiKey, baseUrl);
      _log('Settings saved: baseUrl = $baseUrl');
      notifyListeners();
    } catch (e) {
      _log('Error saving settings: $e');
    }
  }
  ...
  Future<void> loadModels() async {
    try {
      ...
      // Подсчет бесплатных моделей
      _freeModelsCount = _availableModels.where((model) {
        final promptPrice =
            double.tryParse(model['pricing']['prompt'] ?? '0') ?? 0.0;
        final completionPrice =
            double.tryParse(model['pricing']['completion'] ?? '0') ?? 0.0;
