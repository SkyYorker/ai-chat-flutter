...
class OpenRouterClient {
  ...
  // Метод обновления настроек API
  void updateSettings(String newApiKey, String newBaseUrl) {
    apiKey = newApiKey;
    baseUrl = newBaseUrl;
    headers['Authorization'] = 'Bearer $newApiKey';
    if (kDebugMode) {
      print('OpenRouterClient settings updated: baseUrl = $newBaseUrl');
    }
  }
                ...
                (model) => {
                  ...
                  'name': (() {
                    try {
                      ...
                      final cleaned = (model['name'] as String).replaceAll(
                        RegExp(r'[^\x00-\x7F]'),
                        '',
                      );
    ...
    try {
      ...
      final data = {
        ...
        'max_tokens': int.parse(
          dotenv.env['MAX_TOKENS'] ?? '1000',
        ), // Максимальное количество токенов
        'temperature': double.parse(
          dotenv.env['TEMPERATURE'] ?? '0.7',
  ...
  Future<String> getBalance() async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
          baseUrl?.contains('vsegpt.ru') == true
              ? '$baseUrl/balance'
              : '$baseUrl/credits',
        ),
        headers: headers,
